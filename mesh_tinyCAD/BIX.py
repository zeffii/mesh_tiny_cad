...
def add_line_to_bisection(self):
    ...
    [[j1, j2], [j3, j4]] = [[v.index for v in e.verts] for e in edges]
    a, b = {j1, j2}, {j3, j4}
    shared = a & b

    # definitely 'intersects' because shares vertex
    if len(shared) == 1:
        print('shares_vertex!')
        ix1, ix2 = tuple(a ^ b)
        pt = bm.verts[shared.pop()].co
        far1 = bm.verts[ix1].co
        far2 = bm.verts[ix2].co
    else:
        edge1 = (v1, v2)
        edge2 = (v3, v4)
        ...
        # pick furthest vertex from (projected) intersections
